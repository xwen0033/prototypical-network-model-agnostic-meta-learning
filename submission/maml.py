...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):
        ...
        # Compute initial accuracy with the original parameters
        with torch.no_grad():
            initial_logits = self._forward(images, parameters)
            initial_accuracy = util.score(initial_logits, labels)
    ...
    def _outer_step(self, task_batch, train):
        ...
        for task in task_batch:
            ...
            ### START CODE HERE ###
            # Adapt the model parameters using the support set
            adapted_parameters, accuracies_support, _ = self._inner_loop(images_support, labels_support, train=train)
            accuracies_support_batch.append(accuracies_support)

            # Evaluate the adapted model on the query set and compute the loss
            logits_query = self._forward(images_query, adapted_parameters)

            criterion = torch.nn.CrossEntropyLoss()
            loss_query = criterion(logits_query, labels_query)
